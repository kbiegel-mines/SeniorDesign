...
oy = '0' 
oz = '0' 
sgems.execute('NewCartesianGrid gridxy1::'+nx+'::'+ny+'::'+nz+'::'+dx+'::'+dy+'::'+dz+'::'+ox+'::'+oy+'::'+oz) 
varrange = '2' 
searchelips = str(2*int(varrange)) 
sgems.new_point_set('pointset1',samplx,samply,samplz) 
sgems.set_property('pointset1','hardtest1',samplval) 
sgems.execute('RunGeostatAlgorithm  sgsim::/GeostatParamUtils/XML::<parameters>  <algorithm name="sgsim" />     <Simulation_seed type="clock"/>  <Path type="random"><Seed type="clock"/></Path>  <Nb_processors  value="-2"  />    <Grid_Name value="gridxy1" />     <Property_Name  value="simtest1" reuse_property="0" />     <Nb_Realizations  value="3" />     <Max_Conditioning_Data  value="12" />     <Max_Conditioning_Simul_Data  value="12" />     <Search_Ellipsoid  value="'+searchelips+' '+searchelips+' 0  0 0 0" />    <Assign_Hard_Data  value="0"  />     <Hard_Data  grid="pointset1"   property="hardtest1"  />    <Covariance_input  structures_count="1" >    <Structure_1 type="Covariance">  <Two_point_model  contribution="1"  type="Spherical Covariance"   >      <ranges range1="'+varrange+'"  range2="'+varrange+'"  range3="0"   />      <angles azimuth="0"  dip="0"  rake="0"   />    </Two_point_model>    </Structure_1>  </Covariance_input>  </parameters>   ') 
real_319 = [] 
